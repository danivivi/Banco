...
void editar_banco(Banco p[],int lon);
void Mostrar_banco(int lon);
int a=0;
int main()
{	
	if(a==0)
...
void Menu()
{
	...
	do{
		...
		printf("0.Salir\n");
		printf("Que desea hacer Ingrese una Opcion:\n");
		scanf("%d",&opcion);
		switch(opcion)
		{
		case 0:
...
void datos_clientes(cliente v[]){
	...
	if (tam<=100){
		...
		printf("ERROR Limite del Banco 100 Personas.\n");
	}
	printf("-------------------------------------------------\n");
}
...
void cliente_consignar(int tam){
	...
	char c[30]="Consignacion";
	int comparacion;
	printf("Clientes que van a Consignar\n");
	for(int i=0;i<tam;i++){
